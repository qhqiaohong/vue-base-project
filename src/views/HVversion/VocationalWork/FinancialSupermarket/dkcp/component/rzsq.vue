  ...
  <div class="content">
    <div class="c-top">
      <div class="c-t-tab btnActive" v-for="(item, index) in tabs" :key="index" @click="tabFun(item, index)">
        ...
        <div :class="tabIndex == index ? 'c-t-name ' : 'c-t-name'">
          已指定产品： {{ list.productName }}
        </div>
      </div>
    </div>
    <yw-box class="xzcxqy-box">
      <el-form label="left" style="height:100%;overflow-y: scroll;" :inline="true" :rules="rules" ref="form" :label-style="{ 'font-size': '40px' }" :model="from" label-width="220px">
        <el-form-item prop="amount" label="融资金额(万元)"><el-input v-model="from.amount" size="medium" placeholder="请输入融资金额"></el-input>
        </el-form-item>
        <el-form-item prop="loanDuration" label="融资期限(月)">
          <el-input v-model="from.loanDuration" placeholder="请输入融资期限"></el-input></el-form-item>
        <el-form-item prop="useTo" label="融资用途:">
          <el-select v-model="from.useTo" placeholder="请选择">
            <el-option v-for="item in option" :key="item.financeAgency" :label="item.agencyName" :value="item.financeAgency">
            </el-option>
          </el-select>
        </el-form-item>
        <el-form-item prop="realEstate" label="名下房产(企业及法人)">
          ...
          <!-- <el-radio-group v-model="radio">
                <el-radio size="medium"  :border="true" :label="3">公开</el-radio>
                <el-radio size="medium" :border="true"  :label="6">不公开</el-radio>
              </el-radio-group> -->
        </el-form-item>
        ...
        <el-form-item prop="businessAddressDetail" label="详细地址">
          <el-input v-model="from.businessAddressDetail" placeholder="请输入详细地址"></el-input>
        </el-form-item>
        ...
        <el-form-item prop="existingLoanBalance" label="贷款余额">
          <el-input v-model="from.existingLoanBalance" placeholder="请输入融资期限"></el-input></el-form-item>
        <el-form-item prop="contactIdCard" label="联系人身份证号"><el-input v-model="from.contactIdCard" placeholder="请输入联系人身份证号"></el-input>
        </el-form-item>
        <el-form-item prop="contactName" label="联系人姓名"><el-input v-model="from.contactName" placeholder="请输入联系人姓名"></el-input>
        </el-form-item>
        <el-form-item prop="contactPhone" label="联系人电话"><el-input v-model="from.contactPhone" placeholder="请输入联系人电话"></el-input>
        </el-form-item>
        <el-form-item prop="businessPerformance" label="企业经营状况说明(选填)">
          <el-input type="textarea" :autosize="{ minRows: 4, maxRows: 10 }" v-model="from.businessPerformance" placeholder="请输入融资期限"></el-input>
        </el-form-item>
      </el-form>
      <el-checkbox v-model="checked">本人承诺上述信息全部属实，一切由上述信息错误引起的后果均由本人自负。</el-checkbox>
...
import { findFinanceUseTo, publishFinance } from "@/api/login";
      ...
      this.$refs["form"].validate((valid) => {
        if (valid) {
          if (this.checked == true) {
            ...
            // );
            (this.from.appId = "Ytqx873Hgd"),
              (this.from.requestTime = times),
              (this.from.sign = md5(
                `appId=Ytqx873Hgd&appSecret=36048c3b55e811eeb31f0242ac140002&requestTime=${times}`
              )),
              (this.from.ticket = JSON.parse(
                sessionStorage.getItem("inch")
              ).ticket);
            ...
            this.from.productCode = this.list.code;
